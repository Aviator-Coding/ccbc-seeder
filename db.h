...
public:
  ...
  bool IsGood() const {
     if (ip.GetPort() != GetDefaultPort()) {
      //printf("Bad Node - Port %i not matching Default Port %i \n",ip.GetPort(),GetDefaultPort());
      return false;
      }
    if (!(services & NODE_NETWORK)) {
      //printf("Bad Node - Serivces not matching \n");
      return false;
      }
    if (!ip.IsRoutable())
    {
       //printf("Bad Node - Not Routable Node \n");
       return false;
    }
    if (clientVersion && clientVersion < REQUIRE_VERSION)
    { 
      //printf("Bad Node - ClientVersion %i is lower than Requuired Version %i \n", clientVersion, REQUIRE_VERSION);
      return false;
    }
    if (blocks && blocks < GetRequireHeight()){
       //printf("Bad Node - Nodes Block height %i is less the Required Block Height %i \n", blocks, GetRequireHeight());
       return false;
    }
    ...
    {
      //printf("Bad Node - Not Reliable enough yet %i <= 3 && %i * 2 >= %i \n", total, success , total);
    }
